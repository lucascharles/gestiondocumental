...
//echo "nro de pivots ".count($this->_pivotOn);
        switch (count($this->_pivotOn)) {
            case 1:
...
// aca genera la cabecera de las filas
...
// aca pone el valor de cade celda
                                $_out["{$split}{$col}"] = $value;
//
                                if ($this->_lineTotal) {
                                    $_lineTotal[$k] += $value;
                                }
...
//                            $_out[self::TOTAL . "ddddddddddddd{$k}"] = $value;
