<?php
	...
	class SqlSoporte
	{
		...
		function SqlSoporte($h, $u, $p)
		{
			...
			$this->user = $u; 
			$this->pass = $p;
			$this->db_connect_id_s = mysql_connect($this->host, $this->user, $this->pass);
		}
		...
		function load()
		{
			...
			$cant_reg = $this->get_cant_registros(); 
			if($cant_reg >= $this->sql_limit_f)
			{
...
//			echo("<br>sql: ".$sql);
			...
			while($fila=mysql_fetch_array($select))
			{
				...
				{
					$ascol_expl = explode("as",trim($col[$i]));
					
					if(count($ascol_expl) > 1)
					{					
						$arrayaux[trim($ascol_expl[1])] = $fila[trim($ascol_expl[1])];
					}
					else
					{
		...
		function get_cant_registros()
		{
			...
			$cant = mysql_num_rows($select);
			
			return $cant;
		}
		
		function ejecuta_backupsql($sql)
		{
			//echo("<br>sql: ".$sql);
			$select = mysql_query($sql,$this->db_connect_id_s);
		}
		...
		function loadSqlCompleto()
		{
			...
			while($fila=mysql_fetch_array($select))
			{
				...
				$selAux = explode("from",$this->sql_completo);
				$selAux_1 = explode("select",$selAux[0]);
				
				$col = explode(",",$selAux_1[1]);
	...
	//		$sql = "SELECT * FROM " . $data_objects[$this->db_key]->sql_escape_tablename($this->table_name);
		...
		function _update()
		{
			...
			if ($sql_set != "") 
			{
				if(in_array($this->table_name,$_SESSION["tablas_audi"]))
				{
					$sql_fields_audi = ", ".$data_objects[$this->db_key]->sql_escape_tablename("id_usuario").",".$data_objects[$this->db_key]->sql_escape_tablename("fecha_trans").",".$data_objects[$this->db_key]->sql_escape_tablename("tipo_trans");
					$sql_values_audi = ",'". $_SESSION["idusuario"]."', '".date("Y-m-d H:i:s")."', 'U' ";
					//$sql = " INSERT INTO " . $this->table_name . "_audi (" . $sql_fields . $sql_fields_audi . ")";
					$sql = " INSERT INTO " . $this->table_name . "_audi ";
					// VALUES (" . $sql_values . $sql_values_audi . ");";
					$sql .= " SELECT * ". str_replace("`","",$sql_fields) . $sql_values_audi ." FROM ". $data_objects[$this->db_key]->sql_escape_tablename($this->table_name);
					$where_sql = $this->build_where_clause();
					if ($where_sql != "") 
					{
						$sql .= " WHERE" . $where_sql;
		...
		function _insert()
		{
			...
			if ($sql_fields != "")
			{
				...
				{
					$sql_fields_audi = ", ".$data_objects[$this->db_key]->sql_escape_tablename("id_usuario").",".$data_objects[$this->db_key]->sql_escape_tablename("fecha_trans").",".$data_objects[$this->db_key]->sql_escape_tablename("tipo_trans");
					$sql_values_audi = ",'". $_SESSION["idusuario"]."', '".date("Y-m-d H:i:s")."', 'I' ";
					$sql = "INSERT INTO " . $this->table_name . "_audi (" . $sql_fields . $sql_fields_audi . ") VALUES (" . $sql_values . $sql_values_audi . ");";
					$result = $data_objects[$this->db_key]->sql_query($sql);
				}
				//echo("<br>sql: ".$sql);
