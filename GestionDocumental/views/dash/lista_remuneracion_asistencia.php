...
<table width="100%" cellpadding="0" cellspacing="0" align="center" border="0" bgcolor="#FFFFFF" id="listado">
	...
	<tr bgcolor="#FFFFFF" id="fila_<?php echo ($rs["trbIdTrabajador"])  ?>">
        ...
        <? if($rs["trbIdTrabajador"] == 1){ ?>
        		<img src="images/activar.gif" title="Completos" class="oplistado" onclick="abrirLiquidaciones('<? echo($controller) ?>',<?php echo ($rs["trbIdTrabajador"]) ?>)" />
        	<? } else {	?>
        		<img src="images/borrar.gif" title="Completos" class="oplistado" onclick="abrirLiquidaciones('<? echo($controller) ?>',<?php echo ($rs["trbIdTrabajador"]) ?>)" />
        <? } ?>
        </td>
        <td align="left" width="20%">
        <? if($rs["trbIdTrabajador"] == 1){ ?>
        		<img src="images/activar.gif" title="Completos" class="oplistado" onclick="verDetalle('<? echo($controller) ?>',<?php echo ($rs["trbIdTrabajador"]) ?>)" />
        	<? } else {	?>
        		<img src="images/borrar.gif" title="Completos" class="oplistado" onclick="verDetalle('<? echo($controller) ?>',<?php echo ($rs["trbIdTrabajador"]) ?>)" />
        <? } ?>
        </td>
