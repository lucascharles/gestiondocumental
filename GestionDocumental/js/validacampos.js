	 	...
	 	var resultado = true;
  		 ...
  		 if(arrayInput[i].getAttribute('valida') == "requerido")
			...
			{	
				...
				if(clase == "input_form_largo" || clase == "resalta_largo" || clase == "notFilled_largo")
				{
					$(arrayInput[i]).addClass('notFilled_largo');
				}
				if(clase == "input_form_largo ui-autocomplete-input" || clase == "resalta_largo" || clase == "notFilled_largo")
				{
					$(arrayInput[i]).addClass('notFilled_largo');
				}
	 	...
	 	var resultado = true;
  		 if(arraySelect[i].getAttribute('valida') == "requerido")
			...
			if(resultado == false)
			{
				...
				if(clase == "input_form_largo" || clase == "resalta_largo" || clase == "notFilled_largo")
				{
					$(arraySelect[i]).addClass('notFilled_largo');
				}
				if(clase == "input_form_min" || clase == "resalta_min" || clase == "notFilled_min")
				{
			...
			else
			{
				if(arrayInput[i].getAttribute('tipovalida') != null)
				{
					if(arrayInput[i].getAttribute('tipovalida') == "texto")
					{
						...
						{
							if(arrayInput[i].getAttribute('referente') != null)
							{
								if(!validaentero($("#"+arrayInput[i].getAttribute('referente')).val()))
								{
									resultado = false;
								}
							}
...
(function($){
  $.fn.validator = function(opts){
	...
	// PARA INPUT LARGO
	$(this).find(".notFilled_largo").live('keyup', function(){
	

		clase = "notFilled_largo";
		clasep = "resalta_largo";
	...
	function validaDuracion(obj, obj1)
	{
		var dur = new Number(obj.value);
		var avis = new Number(obj1.value);
		if(dur < avis)
		{
			obj1.value = dur;
		}
